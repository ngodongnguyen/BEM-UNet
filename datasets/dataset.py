...
def random_rot_flip(image, label, seed=42):
    np.random.seed(seed)
...
def random_rotate(image, label, seed=42):
    np.random.seed(seed)
...
class RandomGenerator(object):
    def __init__(self, output_size, seed=42):
        self.output_size = output_size
        self.seed = seed

    def __call__(self, sample):
        # reset seed cho mỗi sample → tránh bị thay đổi do worker hoặc batch
        ...
        if image.ndim == 3:
            _, x, y = image.shape      # C,H,W
        else:
            x, y = image.shape         # H,W fallback        
